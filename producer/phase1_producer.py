...
def ensure_ist_index(df):
    """Safely handle tz-naive and tz-aware yfinance output"""
...
if now_ist().time() >= FREEZE and not df_freeze.empty:
    ...
    if after.empty:
        ...
        relation_color = "GREEN" if relation == "SUPPORTING" else "RED"
    ...
    for i in range(1, len(df_freeze)):
        prev = df_freeze.iloc[i - 1]
        cur = df_freeze.iloc[i]
...
# =========================
# WRITE SNAPSHOT (NON-DESTRUCTIVE)
# =========================
