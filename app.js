(async () => {
  ...
  render("box-vwap", "vwap", `
    ...
    <div class="small">
      15‑min candle basis ${safe(d.vwap?.basis_candle_close)}
    </div>
  `);
  ...
  render("box-trend", "trend", `
    ...
    <div class="line">
      <b>Gap Behavior:</b> ${safe(ta.gap_behavior?.status)}
    </div>
    <div class="small">
      Frozen at ${safe(ta.gap_behavior?.frozen_at)}
    </div>
    ...
    <div class="line">
      <b>Next Candle:</b> ${safe(ta.next_candle?.relation)}
    </div>
    ...
    <div class="line">
      <b>Market Character:</b> ${safe(ta.market_character)}
    </div>
  `);
  ...
  render("box-flows", "institutional manual", `
    ...
    <div class="line">
      ...
      FII ${flowFmt(flows.today?.fii)} |
      DII ${flowFmt(flows.today?.dii)}
    </div>

    <div class="small">
      <b>FII (Last 4):</b>
      ${
        flows.history_4d?.fii
          ?.map(x => flowFmt(x.value))
          .join(" | ") ?? "—"
      }
    </div>
  ...
  if (typeof pcrVal === "number") {
    if (pcrVal < 0.8) {
      pcrText = "Bearish options positioning (call‑heavy structure)";
    } else if (pcrVal <= 1.1) {
      pcrText = "Balanced to mildly bullish options positioning";
    } else {
      pcrText = "Bullish options positioning (put‑heavy structure)";
    }
  }
